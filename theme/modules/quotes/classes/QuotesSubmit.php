	...
	public function getQuoteById($id_quote, $id_customer)
	{
		...
		foreach ($products as $item)
		{
			...
			$image_id = getProductAttributeImage($item['id'], $item['id_attribute'], $this->context->language->id) ?
				getProductAttributeImage($item['id'], $item['id_attribute'], $this->context->language->id) : $itemp->id;
			...
			$product_arr[] = array(
				...
				'link'	   => $this->context->link->getProductLink($itemp, $itemp->link_rewrite, $itemp->category, null,
						null, $itemp->id_shop, $item['id_product_attribute']),
				'quantity'   => $item['quantity'],
				'unit_price' => Tools::displayPrice(Tools::ps_round($itemp->getPriceStatic($itemp->id, true,
						$item['id_attribute'], 6), 2), $this->context->currency),
				'total'	  => Tools::displayPrice(Tools::ps_round(($itemp->getPriceStatic($itemp->id, true,
							$item['id_attribute'], 6) * (int)$item['quantity']), 2), $this->context->currency),
			);
